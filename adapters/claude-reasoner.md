...
2. Its thin user-scoped Ariadne skill locates the installed runtime and runs
...
5. It uses `ariadne.py advance` for normal evidence and continuation.
...
says it reads `CLAUDE.md`; Ariadne deliberately avoids adding that
...
- Read and write only the project plus the external Ariadne run directory
...
Record a provider failure through the Ariadne controller. When no material
