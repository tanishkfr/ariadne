# RESEARCH: <name>

> Template. Owner: Researcher · Stage: S2 · Skill: [RESEARCH-POLICY.md](../RESEARCH-POLICY.md)
> Policy: [RESEARCH-POLICY.md](../RESEARCH-POLICY.md)
> **Every claim carries a claim status, date, URL, and source-confidence level.** Definitions, freshness windows, source
> ranking, and how to handle disagreement all live in [RESEARCH-POLICY.md](../RESEARCH-POLICY.md) — this is just the register.

**Last updated:** <date>

---

Claim status: **OBSERVED** · **SUPPORTED** · **INFERRED** · **HYPOTHESIS** · **ASSUMPTION**.

Source confidence: **Verified** · **Reported** · **Unverified** · **Contradicted** — see [RESEARCH-POLICY.md](../RESEARCH-POLICY.md) section 4.

---

## Findings

| # | Claim | Claim status | Date | Source confidence | Source |
|---|---|---|---|---|---|
| 1 | <a falsifiable statement, not a question> | OBSERVED | <YYYY-MM-DD> | Verified | <url> |

> Write claims, not topics.
> Topic: "Cursor pricing." Claim: "Cursor's India individual plan is ₹650/month."

## Contradictions

| Claim | Version A | Version B | Acting on | Why |
|---|---|---|---|---|
| | <source, date> | <source, date> | <which> | <> |



## Could not verify

| Question | Blocker | What would resolve it |
|---|---|---|
| | <paywall / 403 / region lock / login> | <> |

> **"I could not verify this" is a complete, valid output.**

## Component and technique research

> From [component-research](../skills/component-research.md). The technique, not the dependency.

| Interaction | Technique | Source | Build or install | Why |
|---|---|---|---|---|
| | <what is actually happening> | <url> | build / install | |

## Library checks

> Before any G2. `npm view <pkg> version time.modified license`

| Package | Version | Last release | Licence | Maintained? | Weight (gzip) | Checked |
|---|---|---|---|---|---|---|
| | | <date> | | yes/no | | <date> |

## Reference analysis

> From [reference-analysis](../skills/reference-analysis.md). Mechanisms, not surfaces. Max 2 per reference.

| Reference | Organising principle | Type | Colour | Motion | Absent | Why it feels expensive |
|---|---|---|---|---|---|---|
| <url> | | | | | | |

**Pooled mechanisms** (attribution dropped):

1. <>
2. <>

**The tension to aim for:** <two opposed qualities>

## Research budget

| Question | Budget | Used | Outcome |
|---|---|---|---|
| | <1-2 / 5-8 / 10-15 lookups> | | resolved / reported unresolved |

> Set the budget before searching, and stop when it is spent ([RESEARCH-POLICY.md](../RESEARCH-POLICY.md) section 2).
